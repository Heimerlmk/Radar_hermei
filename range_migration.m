...
for k=1:1:K                               %总共K个目标
   ...
   Srnm=Srnm+sigma*exp(1j*phase).*(0<Dfast&Dfast<Tr).*((abs(Dslow)<Lsar/2)'*ones(1,M));%由于是多个目标反射的回波，所以此处进行叠加
end
...
tr=tm-2*Rmin/C;
Refr=exp(1j*pi*Kr*tr.^2).*(0<tr&tr<Tr);
...
%%方位向压缩
ta=sn-Xmin/V;
Refa=exp(1j*pi*Ka*ta.^2).*(abs(ta)<Tsar/2);
...
imagesc(row,col,Gr);           %距离向压缩，未校正距离徙动的图像
...
imagesc(row,col,Ga);          %距离向压缩，校正距离徙动后的图像
